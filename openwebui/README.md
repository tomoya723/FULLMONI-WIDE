...
## 17. 記憶が参照されない件（max_chars_total による打ち切り）

### 症状

`FULLMONI-Legacy ってどういう製品だった？` と聞くと、`projects.md` に正しい説明が
書いてあるのに、富士通／富士ソフトのネットワーク監視ソフトという実在の別製品の話を
でっち上げる。検索トグルの ON/OFF に関わらず再現した。

### 真因

`memory_file_filter` の Valves がこうなっていた。
...
`projects.md` 1,844 字なので、**profile.md の途中で打ち切られ projects.md は
1 文字も注入されていなかった**。FULLMONI-Legacy は projects.md の末尾にある。

モデルの「提供された検索結果や記憶には記載されていない」という応答は**正確**だった。
記憶が効いていないのではなく、記憶に入っていなかった。

`files` に `inbox.md` が無い点も同時に問題で、これだと夜間バッチ（§16）が追記した
新事実が、週 1 の統合が走るまで最大 7 日間読まれない。
...
| `max_chars_per_file` | (既定 6000) | 8000 | 1 ファイルで打ち切られないように |
| `files` | `profile.md,projects.md` | `+inbox.md` | 夜間バッチの追記を即日反映する |
| `web_search_files` | (既定 空) | 3 ファイル | 検索ターンでも自分の情報を答えられるように |

記憶ブロックは system prompt の先頭に置いてあるので、増えた分は llama-server の
prefix cache に乗る。初回ターンだけプリフィルが増え、2 ターン目以降は効かない。

### 教訓

Valves は DB 側に保存されており、**コード側の既定値（`max_chars_total` の既定は
12000）とは無関係**に効く。フィルタのコードをいくら読んでも原因は分からなかった。
デプロイスクリプトが更新前の Valves を出力していたおかげで判明した。

### 検証（2026-09-07 19:30）

デプロイ後、地球マーク OFF で同じ質問をした結果:
...
`projects.md` の記述と完全に一致。記憶が参照されるようになった。

同時に、この一連の作業で以下も解消している。

| 症状 | 原因 | 対処 |
|---|---|---|
| PWA で画面が固まり通知経由で応答が届く | WebSocket が繋がらず session_id が空 | （下記の副作用として解消） |
| 検索トグル OFF でも地球マークが点く | `force_web_search` が session_id 不在を「API 直叩き」と誤判定 | 判定を `chat_id` に変更（§16 追記） |
| 記憶にあるはずの事実を答えない | `max_chars_total` 1500 による打ち切り | 12000 に拡大、`inbox.md` を追加 |

### 残作業

- 両フィルタの `debug` を `true` にしてある。切り分けが済んだので、
  ログのノイズが気になれば `false` に戻す（`deploy-filters.ps1` の `$targets` を編集）。

---

## 18. 作業記録

日付ごとの作業記録は別ファイルに分けてある。README は仕組みと真因の説明、
作業記録は「その日に何をどの順で調べ、どこで詰まったか」を残す。

| ファイル | 内容 |
|---|---|
| [`SESSION_2026-09-07.md`](SESSION_2026-09-07.md) | 記憶の自動蓄積（夜間バッチ）の構築と、記憶が使われない3症状の解決。Filter デプロイスクリプトの新設 |
