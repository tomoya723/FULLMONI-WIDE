...
Macro definitions
******************************************************************************/
#define     LCD_CH0_OUT_CLK_DIV_RATIO     (GLCDC_PANEL_CLK_DIVISOR_12)    /* Division Ratio of LCD Panel Clock                 */

#define     LCD_CH0_W_VSYNC               (0u)                    /* Vertical Pulse Width(VPW)                                   */
#define     LCD_CH0_DISP_VS               (0u + 96u)                    /* LCD display area vertical start position
                                                                 = Vertical Pulse Width(VPW) + Vertical Back Porch(VBP)     */
#define     LCD_CH0_DISP_VW               (256u)                   /* Vertical Display Period(VDP)                                */

#define     LCD_CH0_W_HSYNC               (0u)                    /* Horizontal Pulse Width(HPW)                                 */
#define     LCD_CH0_DISP_HS               (0u + 141u)                    /* LCD display area horizontal start position
                                                                 = Horizontal Pulse Width(HPW) + Horizontal Back Porch(HBP) */
#define     LCD_CH0_DISP_HW               (800u)                   /* Horizontal Display Period(HDP)                              */

#define     LCD_CH0_SIG_FV                (354u)              /* Vertical Total Period(VTP)                                  */
#define     LCD_CH0_SIG_FH                (944u)             /* Horizontal Total Period(HTP)                                */

#define     LCD_CH0_W_VFP                 (2u)             /* Vertical Front Porch(VFP)                                   */
#define     LCD_CH0_W_HFP                 (3u)             /* Horizontal Front Porch(HFP)                                 */

#define     LCD_CH0_W_VBP                 (96u)             /* Vertical Back Porch(VBP)                                  */
#define     LCD_CH0_W_HBP                 (141u)             /* Horizontal Back Porch(HBP)                                */

#define     LCD_CH0_TCON_PIN_VSYNC        (GLCDC_TCON_PIN_NON)      /* Select TCON of a Vsync signal (Vsync_TCON_select)           */
#define     LCD_CH0_TCON_PIN_HSYNC        (GLCDC_TCON_PIN_NON)      /* Select TCON of a Hsync signal (Hsync_TCON_select)           */
...
#define     LCD_CH0_OUT_COLOR_ORDER       (GLCDC_COLOR_ORDER_BGR)    /* Select Pixel Order of Output Data                           */

#define     LCD_CH0_IN_GR1_VSIZE          (256u)                 /* Vertical width of image ddta (Graphic1)                    */
...
#define     LCD_CH0_IN_GR2_VSIZE          (256u)                 /* Vertical width of image ddta (Graphic2)                    */
...
#define     LCD_CH0_IN_GR2_LINEOFFSET     (1600u)                /* Macro line offset (Graphic2)                              */
