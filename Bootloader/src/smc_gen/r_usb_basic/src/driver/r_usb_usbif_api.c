...
usb_err_t R_USB_Open( usb_ctrl_t *ctrl, usb_cfg_t *cfg )
{
    ...
    else
    {
        ...
        if (USB_SUCCESS == err)
        {
            ...
            /* Force attach process immediately - bypass VBUS detection */
            /* R26 is not mounted in peripheral mode, so VBUS detection doesn't work */
            usb_cpu_delay_xms((uint16_t)10);
            usb_pstd_attach_process();  /* This will enable D+ pull-up */
        }
#endif  /* (USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI */
    }
