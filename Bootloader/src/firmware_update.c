/*
 * firmware_update.c
 * RX72N Bootloader - Firmware Update Implementation
 */
...
/* UART送信（extern参照） */
extern void uart_send_polling(const char *str);

/* CRC32テーブル（標準ポリノミアル 0xEDB88320） */
 ...
 * UART受信（ブロッキング）
 */
static uint8_t uart_receive_byte_blocking(void)
{
    /* エラーチェックとクリア */
    if (SCI9.SSR.BIT.ORER) {
        SCI9.SSR.BIT.ORER = 0;  /* オーバーランエラークリア */
    }
    if (SCI9.SSR.BIT.FER) {
        SCI9.SSR.BIT.FER = 0;   /* フレーミングエラークリア */
    }
    if (SCI9.SSR.BIT.PER) {
        SCI9.SSR.BIT.PER = 0;   /* パリティエラークリア */
    }
...
{
    uint32_t index = 0;
    static bool skip_lf = false;  /* CRLF対策 */

    while (index < MOT_LINE_MAX_LENGTH - 1) {
        uint8_t ch = uart_receive_byte_blocking();  /* ブロッキング受信 */

        /* 前回CRを受信していた場合、LFをスキップ */
        if (skip_lf && ch == '\n') {
            skip_lf = false;
            buffer[index] = '\0';
            return true;
...
/*
 * MOT S3レコードからアドレスとデータを抽出
 */
static bool parse_s3_record(const char *line, uint32_t *address, uint8_t *data, uint32_t *length)
{
    ...
    /* バイト数 */
    uint8_t byte_count;
    sscanf(&line[2], "%2hhx", &byte_count);

    /* アドレス（32bit） */
    sscanf(&line[4], "%8lx", address);

    /* データ長 = バイト数 - アドレス(4) - チェックサム(1) */
    *length = byte_count - 5;

    /* データ抽出 */
    for (uint32_t i = 0; i < *length; i++) {
        sscanf(&line[12 + i * 2], "%2hhx", &data[i]);
    }
...
bool flash_write_data(uint32_t address, const uint8_t *data, uint32_t length)
{
    ...
    /* 書き込み（256バイト境界必須） */
    err = R_FLASH_Write((uint32_t)data, (uint32_t)address, length);
    if (err != FLASH_SUCCESS) {
        return false;
    }
...
fw_update_result_t firmware_update_process(void)
{
    ...
    /* 最初の1行目を受信してデバッグ表示 */
    uart_send_polling("Receiving first line...\r\n");
    ...
    uart_send_polling("First line received: ");
