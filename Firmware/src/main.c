...
#define SCI9_SCMR           (*(volatile uint8_t *)(SCI9_BASE + 0x06))
#define SCI9_SEMR           (*(volatile uint8_t *)(SCI9_BASE + 0x07))

#define PORTB_PDR           (*(volatile uint8_t *)(0x0008C001))
#define PORTB_PMR           (*(volatile uint8_t *)(0x0008C061))
#define MPC_PB7PFS          (*(volatile uint8_t *)(0x0008C19F))  /* 正しいアドレス */
#define MPC_PB6PFS          (*(volatile uint8_t *)(0x0008C19E))  /* 正しいアドレス */
#define MPC_PWPR            (*(volatile uint8_t *)(0x0008C11F))

#define SYSTEM_PRCR         (*(volatile uint16_t *)(0x000803FE))
#define SYSTEM_MSTPCRB      (*(volatile uint32_t *)(0x00080014))
#define MSTPB22             (1 << 22)

void test_sci9_init(void)
{
    /* プロテクト解除 (PRC1ビット) */
    SYSTEM_PRCR = 0xA502;
    ...
    SCI9_SCR = 0x00;
    SCI9_SMR = 0x00;    /* 非同期、8bit、パリティなし */
    SCI9_SCMR = 0xF2;
    SCI9_SEMR = 0x00;
    SCI9_BRR = 7;       /* 115200bps @ 60MHz */
...
void test_sci9_putc(char c)
{
    /* 初期化完了待ち（最大10回リトライ） */
    int retry = 10;
    while (!(SCI9_SSR & 0x80) && retry > 0) {
        for (volatile int i = 0; i < 100000; i++);
        retry--;
    }

    if (retry == 0) return;  /* タイムアウト */

    SCI9_TDR = c;
    SCI9_SSR &= ~0x80;
}

void test_sci9_puts(const char *str)
{
    while (*str) {
        test_sci9_putc(*str++);
    }
